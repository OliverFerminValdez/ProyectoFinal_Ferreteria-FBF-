...
namespace Ferreteria_FBF_App.Pages
{
    public class FacturaDeVenta
    {
        ...
        public void FacturaHeader()
        {
            ...
            pdfCell = new PdfPCell(new Phrase((ClientesBLL.Buscar(venta.ClienteId).Nombre + ClientesBLL.Buscar(venta.ClienteId).Apellido), fontFecha));
            pdfCell.Colspan = maxColumn;
            pdfCell.Border = 0;
            pdfPTable.AddCell(pdfCell);

            pdfCell = new PdfPCell(new Phrase((ClientesBLL.Buscar(venta.ClienteId).Nombre + ClientesBLL.Buscar(venta.ClienteId).Apellido), fontFecha));
            pdfCell.Colspan = maxColumn + 1;
            pdfCell.Border = 0;
            pdfPTable.AddCell(pdfCell);
